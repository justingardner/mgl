 ...
 * @property {Boolean} usingVisualAnglesStencil tells if you are currently drawing in visual angles for your stencil
...
function jglClearScreen(background) {
	if (arguments.length != 0) {
		...
		if ($.isArray(background)) {
			...
			if (background <= 1) { background = Math.round(background * 255); }
...
/**
 * Function to replicate an array 
 * by concating it onto itselt given number of times
 * @param {Array} array 1D array that will be replicated
 * @param {Number} reps, number of step the step between two elements of the array
 * @param {Number} high the high bound of the array
 */
function jglRepmat(array, reps) {
	out = [];
	for (i=0; i<reps; i++) {
