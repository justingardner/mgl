...
/**
* Randomly shuffle array with Fisher-Yates method
* Adapted from http://bost.ocks.org/mike/shuffle
* @param {Array} array 1D array
* @returns {Array} array 1D shuffled array
*/ 
function shuffle(array) {
  var nLeft = array.length; // 
  var pick; // Index of element that will be picked out of remaining set
  var tmp;  // Temporarily story value that will be swapped 

  // Loop through each element of array
  while (nLeft) {
    // Pick a remaining element…
    pick = Math.floor(Math.random() * nLeft--);
    // And swap it with the current element.
    tmp = array[nLeft];
    array[nLeft] = array[pick];
    array[pick] = tmp;
  }
  return array;
}
