...
% check global to see if we have a cached version
global globalHostname

if isempty(globalHostname)
  ...
  if (retval == 0)
    ...
    if (retval == 0)
      % find the matching last characters
      % this is necessary, because matlab's system command
      % picks up stray keystrokes being written into
      % the terminal but puts those at the beginning of
      % what is returned by system. so we run the
      % command twice and find the matching end part of
      % the string to get the hostname
  ...
  % save to make it faster next time
